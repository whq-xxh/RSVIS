...
class MultimodalTransformer(nn.Module):
    def __init__(self, num_encoder_layers=3, num_decoder_layers=3,
        ...
        # self.tokenizer = RobertaTokenizerFast.from_pretrained("/home/whq/HKUSTGZ/RVOS/MTTR-Miccai/roberta-base")   #
        ...
        self.txt_proj_w = FeatureResizer(
            input_feat_size=512,
            output_feat_size=self.d_model,
            dropout=kwargs['dropout'],
        )
        ...
        self.lin1=FeatureResizer(
            input_feat_size=20,
            output_feat_size=4,
            dropout=kwargs['dropout'],
        )
        self.lin2=FeatureResizer(
            input_feat_size=16,
            output_feat_size=4,
            dropout=kwargs['dropout'],
        )
        self.lin3=FeatureResizer(
            input_feat_size=512,
            output_feat_size=1,
            dropout=kwargs['dropout'],
        )
    ...
    def forward(self, vid_embeds, vid_pad_mask, text_queries, obj_queries, obj_w):
        ...
        txt = txt.permute(2,1,0)
        guid = torch.cat((torch.cat((txt_memory,torch.zeros(16-txt_memory.shape[0],t*b,256).cuda()),dim=0),(txt+obj_w).cuda()),dim=0)
        guid = guid.permute(2,1,0)
        guid = self.lin1(guid)
        guid = guid.permute(2,1,0)
        # print("----------------------------------guid")
        # print(guid.shape)
        obj_w = obj_w+F.softmax(guid)*obj_w

        encoder_src_seq = torch.cat((vid_embeds, txt_memory,obj_w), dim=0)  # @whq miccai
        ...
        # print("encoder_src_seq.shape=",encoder_src_seq.shape)
        # print(obj_w.shape[0])

        # seq_mask = torch.cat((rearrange(vid_pad_mask, 't b h w -> (t b) (h w)'), txt_pad_mask), dim=1)
        seq_mask = torch.cat((rearrange(vid_pad_mask, 't b h w -> (t b) (h w)'), txt_pad_mask, torch.zeros([obj_w.shape[1],obj_w.shape[0]]).cuda() ), dim=1)   #@whq miccai

        # vid_pos_embed is: [T*B, H, W, d_model]
        vid_pos_embed = self.pos_encoder_2d(rearrange(vid_pad_mask, 't b h w -> (t b) h w'), self.d_model)

        pos_embed = torch.cat((rearrange(vid_pos_embed, 't_b h w c -> (h w) t_b c'), torch.zeros_like(txt_memory),torch.zeros_like(obj_w)), dim=0)   #@whq miccai
        ...
        # print("memory[:h*w, :, :]=",memory[:h*w, :, :].shape)
        ...
        # print("txt_memory=",txt_memory.shape)
    ...
    def forward_text(self, text_queries, device):
        ...
        # print("tokenized_queries.type=",type(tokenized_queries))
        ...
        # print("txt_memory_ori.shape=",txt_memory.shape)
