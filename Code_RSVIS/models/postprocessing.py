...
class ReferYoutubeVOSPostProcess(nn.Module):
    ...
    def forward(self, outputs, videos_metadata, samples_shape_with_padding):
        ...
        pred_trajectory_indices = torch.argmax(trajectory_scores, dim=-1)    # obj_50 50中选一个 @whq

        # trajectory_scores = prob_is_referred[0][0].max(dim=-1)
        # pred_trajectory_indices = torch.argmax(trajectory_scores.values, dim=-1)   #@ whq 进行改动

        pred_masks = rearrange(outputs['pred_masks'], 't b nq h w -> b t nq h w')
        # keep only the masks of the chosen trajectories:
        b = pred_masks.shape[0]
        pred_masks = pred_masks[torch.arange(b), :, pred_trajectory_indices] #将【1，1，50，90，193】中被选中的pred_trajectory_indices提出来【1，1，90，193】
        ...
        pred_masks = (pred_masks.sigmoid() > 0.5)  #[1,1,360,450] @whq
        ...
        for video_pred_masks, video_metadata in zip(pred_masks, videos_metadata):
            ...
            resized_h, resized_w = video_metadata['resized_frame_size']   #'resized_frame_size': source_frames.shape[-2:] @whq
